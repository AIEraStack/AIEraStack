...
import { getSentryOptions, withSentryRequest } from './lib/sentry';
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  const sentryOptions = getSentryOptions(context.locals.runtime?.env);
  const executionContext = context.locals.runtime?.ctx;
  ...
  const handleRequest = async () => {
    ...
    if (cache) {
      ...
      if (response.status === 200) {
        ...
        executionContext?.waitUntil?.(cache.put(cacheKey, cachedResponse));
  ...
  return withSentryRequest(sentryOptions, request, executionContext, handleRequest);
});
